...
begin
  ...
  if found then
    ...
    values (auth.uid(), p_topic_id, p_stance, p_duration_seconds, created_room_id, 'matched', now() + interval '10 minutes');    return query select 'matched'::text, created_room_id, opponent_entry.user_id, (select p.display_name from public.guest_profiles p where p.user_id = opponent_entry.user_id), opponent_entry.stance, 2::smallint;
    return;
  end if;
...
begin
  perform public.assert_current_user_can_participate();  select r.* into room from public.debate_rooms r where r.id = p_room_id for update;
  ...
  if ready_count = 2 and room.status = 'ready' then
    update public.debate_rooms r set status = 'live', started_at = now() where r.id = p_room_id returning r.* into room;
